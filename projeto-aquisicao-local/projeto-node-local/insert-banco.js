...
function iniciar_escuta() {
    ...
    porta_serial.list().then(entradas_seriais => {
        ...
        var entradas_seriais_arduino = entradas_seriais.filter(entrada_serial => {
            console.log('entrada_serial.vendorId / entrada_serial.productId', `${entrada_serial.vendorId} / ${entrada_serial.productId}`);
...
function registrar_leitura(temperatura, umidade) {
    ...
    banco.conectar().then(() => {
        ...
        return banco.sql.query(`INSERT into dados_arduino (temperatura, umidade, data_e_hora)
